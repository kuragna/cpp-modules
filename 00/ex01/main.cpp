...
void	PhoneBook::print_contact(void)
{
	...
	std::cin >> idx;
	if ((idx >= 0 && idx <= 8) && get_top() == 0)
	{
		std::cout << "INFO: list is empty add a new contact using ADD command\n";
		return ;
	}
...
void	PhoneBook::search(void)
{
	...
	for (int i = 0; i < 4; i += 1)
	{
		std::cout << std::setfill('-') << std::setw(10) << strs[i];
		if (i != 3)
			std::cout << " | ";
	}
	std::cout << std::endl;
	for (int i = 0; i < get_top(); i += 1)
	{
		...
		for (int j = 0; j < 3; j += 1)
		{
			...
			if (s.length() >= 10)
			{
				std::cout << std::setfill('-') << std::setw(9) << s.substr(0, 9);
				std::cout << ".";
			}
			else
...
Contact	Contact::get_info(void)
{
	...
	for (int i = 0; i < contact.size; i += 1)
	{
		std::cout << "Enter your " << infos[i] << ": ";
		std::cin >> contact.infos[i];
	}
